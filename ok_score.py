...
def ress_2line_(u, x):
    u1, u2 = u
    line1_loss = ress_1line(u1, x)
    line2_loss = ress_1line(u2, x)
    return np.where(np.abs(line1_loss) < np.abs(line2_loss),line1_loss,line2_loss)

def ress_1line_vec(u_vec, x):    
    '''
    Args:
        u_vec: 1-dim array of u
    Returns:
        1-dim array containing loss for each u
    '''
    return ress_1line(u_vec[:,np.newaxis], x)
    
def grid_search_(func, x, bounds, res=100):
    search_list = tuple([np.linspace(b[0],b[1],res)])
    grid = np.meshgrid(*search_list)
    comb = np.array([ax.ravel() for ax in grid])
...
def do_score(pygor,point,resmin=20,res2=60,resmax=128,size=100,gates=["c3","c4","c5","c6","c7","c8","c9","c10"],varz=['c5','c9'],filt=1e-11):
    ...
    if do_scale: traces = scaler(traces)
    ...
    ok_score = ok_score2 #np.maximum(ok_score1,ok_score2)
    ...
    if True:
        ...
        ok_score = ok_score_better2 #np.maximum(ok_score_better1,ok_score_better2)
